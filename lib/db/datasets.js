...
export async function deleteDataset(datasetId) {
  try {
    ...
    // 但需要先检查问题是否存在（本地导入的数据集可能使用随机ID，对应的问题可能已被删除）
 ...
 * 获取确认统计
 */
export async function getConfirmationStats(projectId) {
  try {
    const [confirmed, unconfirmed] = await Promise.all([
      db.datasets.count({ where: { projectId, confirmed: true } }),
      db.datasets.count({ where: { projectId, confirmed: false } })
    ]);
    return { confirmed, unconfirmed };
  } catch (error) {
    console.error('Failed to get confirmation stats:', error);
