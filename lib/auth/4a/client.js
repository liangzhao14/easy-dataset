...
/**
 * 调用一个中台网关端点。signInfo 里的 appMethod 用配置的固定值（BS 指南样例 /authcenter/getOriginalForSign）。
...
async function gatewayCall(url, params) {
  ...
  const endpoint = new URL(url).pathname; // 仅用于日志
  // appMethod 进 signInfo：用配置的固定值（BS 指南样例 /authcenter/getOriginalForSign），非业务端点路径
  const headers = buildGatewayHeaders(fourAConfig.appMethod);
  ...
  try {
    ...
    throw new Error(`[4a] ${endpoint} 返回非 JSON (HTTP ${res.status})：${(res.body || '').slice(0, 200)}`);
  }
  // 错误应答：{ error, error_description }（token/userinfo）
  if (json.error) {
    throw new Error(`[4a] ${endpoint} 失败：${json.error} ${json.error_description || ''}`);
  }
  return json;
}
